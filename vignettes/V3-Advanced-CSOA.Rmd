...
printStats <- function(seuratObj, col1, col2){
  print(paste0('Pearson correlation: ', cor(seuratObj@meta.data[[col1]], 
                                            seuratObj@meta.data[[col2]])))
...
```{r, warning=FALSE, out.height='60%', out.width='60%', fig.height=4, fig.width=5}
nReplacedGenes <- 12
...
```{r, warning=FALSE, out.height='60%', out.width='60%', fig.height=7, fig.width=6}
components <- c(1, 2)
...
```
This function has reduced the number of overlaps from 760 to 486 by selectively removing overlaps for which the neighbors of the corresponding genes overlapped little, as calculated using the Jaccard score. The resulting graph has two connected components, which correspond to acinar and ductal cells, respectively:

```{r, message=FALSE, warning=FALSE, out.height='60%', out.width='60%', fig.height=7, fig.width=6}
dfNetwork <- qGrab('mixedPairs.qs')
dfNetwork <- connectedComponents(dfNetwork)
dplyr::count(dfNetwork, component)
...
To display gene participation in top overlaps, CSOA provides the `geneCirclePlot` function:
